...
for _ in range(10,100,5):
    ...
    batch_image0, gt_noise = get_image0(args,_)
    image0=batch_image0[_:_+5]
    print(image0)
    image0 = torch.tensor(image0).permute(0,3,1,2).cuda().clamp(0, 1).float()
    print(image0.shape)
    #exit()
    ...
    for i in range(args.num_iter):
        ...
        if args.mixed_precision:
            ...
            image = from_noise_to_image(args,args.cur_model,cur_noise,args.model_type)
            loss = criterion(image,image0).mean()
    ...
    # 将 PyTorch 张量保存为图像文件
        ...
        print(criterion(image,image0).mean(-1).mean(-1).mean(-1))
        min_value = criterion(image,image0).mean(-1).mean(-1).mean(-1).min()
        mean_value = criterion(image,image0).mean()
        print("min: ",min_value)
