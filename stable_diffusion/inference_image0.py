...
def get_image0(args,times):
    ...
    if args.input_selection == "use_shiba_image0":
        shiba_img = cv2.imread("/hy-tmp/ronan1/cute_shiba_fake/cute_shiba_fake_{}.png".format(times))
        ...
        shiba_img_show.save("cute_shiba/shiba_img_show{}.jpg".format(times))
    ...
    if args.input_selection == "use_generated_image0":
        with torch.no_grad():
            if args.model_type in ["pd","pd_unet"]:
    ...
    # 将 PyTorch 张量保存为图像文件
        #            save_img_tensor(sample_image, f'./conss_model/sample_{ip+1}_times_{times}.png')
                ...
                for i in range(10):
                    ...
                    latents = torch.randn((1, 4, 64, 64), device=device, generator=torch.Generator(device=device))
                    latents *= args.cur_model.scheduler.init_noise_sigma
                    ...
                    with torch.no_grad():
                        image0=args.cur_model.decode_latents(latents.detach())        
                        image0=args.cur_model.numpy_to_pil(image0)[0]
                        image0.save("./original_image0_0204/image0_{}.png".format(i))
